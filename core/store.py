"""Generic JSON store: GitHub Contents API + local-file fallback.

Same shape as oncura-programs / oncura-apps. Only the GITHUB_REPO constant
changes between apps.
"""
...
GITHUB_REPO = os.environ.get("ONCURA_BILLING_REPO", "alexanderjordain/oncura-billing")
GITHUB_BRANCH = os.environ.get("ONCURA_BILLING_BRANCH", "main")
...
def load_json(rel_path: str, default=None):
    """Load data/<rel_path>. Returns (data, sha)."""
    if _github_token():
...
def save_json(rel_path: str, data, message: str, sha: str | None = None, retries: int = 3):
    """Commit to GitHub data/<rel_path>. Always writes the local copy too.
    Returns (ok, info)."""
    ...
    return False, "GitHub kept rejecting our save after retries — try again."
